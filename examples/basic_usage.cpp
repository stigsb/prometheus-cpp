...
int main() {
    ...
    auto& requests =
        registry.counter<AppLabels>("http_requests_total", "Total HTTP requests")
            .required(AppLabels::Key::service, AppLabels::Key::method, AppLabels::Key::status_code)
            .const_label("env", "production")
            .build();

    auto& latency =
        registry.histogram<AppLabels>("http_request_duration_seconds", "Request latency")
    ...
    auto& active = registry.gauge<AppLabels>("active_connections", "Currently open connections")
